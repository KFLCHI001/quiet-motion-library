...
To evaluate low-cost idle loops for an AI-generated mascot still, use the [Generated mascot motion skill](skills/generated-mascot-motion/SKILL.md) and its scripts in `tools/mascot/`: an image-to-video loop on a chroma frame, exported per platform (WebM with alpha, stacked-alpha MP4, HEVC-with-alpha on macOS) with a manifest, and a frame-swap Lottie built from aligned image edits. It includes stage-by-stage questions and a character card template. Its [routes and costs](skills/generated-mascot-motion/references/routes-and-costs.md) record a measured comparison. Generation calls need your own provider keys and are billed by those providers.
