...
| `native/` | Reanimated kept mark (plain Views) and signal field adapter; motion-policy hook | Source only; reuses `examples/signal-field/model.js` | Code, not artwork (MIT). The kept mark is a simple View shape, not the bookmark SVG. Native rendering unverified. |
